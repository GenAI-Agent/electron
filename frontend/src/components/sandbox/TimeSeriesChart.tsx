...
export const TimeSeriesChart: React.FC<TimeSeriesChartProps> = ({ data, keyEvents = [] }) => {
  ...
  return (
    <div className="bg-white rounded-lg p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">支持度變化趨勢</h3>
      ...
      <div className="flex items-center justify-center space-x-8 mb-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-600">候選人A</span>
        </div>
        <div className="flex items-center space-x-2">
          <div className="w-4 h-1 bg-blue-500"></div>
          <span className="text-sm text-gray-600">候選人B</span>
        </div>
        <div className="flex items-center space-x-2">
      ...
      {keyEvents.length > 0 && (
        <div className="mt-6">
          <h4 className="text-md font-medium text-gray-900 mb-3">關鍵事件</h4>
          <div className="space-y-2">
            {keyEvents.map(event => (
              <div key={event.id} className="flex items-start space-x-3 text-sm">
                <span className="text-gray-500 w-12">第{event.week}週</span>
