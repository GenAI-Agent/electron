...
ELECTRON_API_URL = "http://localhost:4080/browser-action"
...
async def browser_navigate_tool(url: str) -> str:
    ...
    try:
        ...
        result = await call_frontend_browser({"action": "navigate", "url": url})
...
async def browser_click_tool(selector: str) -> str:
    ...
    try:
        ...
        result = await call_frontend_browser({"action": "click", "selector": selector})
...
async def browser_get_page_data_tool() -> str:
    ...
    try:
        ...
        result = await call_frontend_browser({"action": "get_page_data"})
        ...
        if result.get("success"):
            ...
            if links and len(links) > 0:
                response += (
                    f"**注意**: 頁面包含 {len(links)} 個連結，已整合在上方內容中\n"
                )
...
async def browser_read_element_tool(selector: str) -> str:
    ...
    try:
        ...
        result = await call_frontend_browser(
            {"action": "read_element", "selector": selector}
        )
...
async def browser_screenshot_tool() -> str:
    ...
    try:
        ...
        result = await call_frontend_browser({"action": "screenshot"})
...
async def browser_execute_script_tool(script: str) -> str:
    ...
    try:
        ...
        result = await call_frontend_browser(
            {"action": "execute_script", "script": script}
        )
